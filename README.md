...
**Подгонка под подписи — только по просьбе** (`fitlabels.ts`). Геометрия берётся с картинки. Если размеры по картинке и по подписям всё же расходятся, отчёт это показывает, а кнопка «Подогнать под подписи и заменить» сдвигает оси стен так, чтобы ширина и глубина подписанных комнат сошлись с числами: ось тянется как резинка, поправка ложится на комнаты, а не на толщину стен; подпись, что расходится с картинкой больше 6 %, не применяется.
