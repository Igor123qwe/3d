...
**Распознавание плана.** Модель со зрением читает картинку целиком: стены, двери, окна, названия комнат, подписанные площади и размерные цепочки. По цепочкам масштаб ставится сам — калибровать линейкой не нужно. Если цепочек на плане нет, масштаб подбирается по подписанным площадям: комнаты строятся, их площадь сравнивается с подписью, чертёж множится на поправку. Поправка больше чем вдвое отвергается — значит, распознано не то.
...
| Задача | Цепочка моделей | Потолок ответа |
| --- | --- | --- |
| План с картинки | дешёвая со зрением → средняя → сильная | 8000 токенов |
| Товар со страницы | самая дешёвая → следующая | 700 токенов |
| Расстановка мебели | дешёвая с рассуждением → средняя | 4000 токенов |
| Тип предмета по названию | самая дешёвая | 120 токенов |
...
3. Локально: `cp .env.example .env` и туда же ключ — `npm run dev` поднимает и серверную часть (плагин `tools/vite-api.ts`).
...
1. **Своя схема.** «Файл» → «Загрузить схему картинкой…», выберите скан или скриншот плана. Нажмите «Калибровать», покажите отрезок с известным размером и введите его длину. Затем «Распознать стены».
2. **Схема.** «Файл» → «2-комнатная»: план, комнаты с площадями, двери с сектором открывания. Нарисуйте свою комнату инструментом «Комната», поставьте дверь и окно. Выберите стену и введите длину — соседние стены подстроятся.
...
5а. **С ИИ.** Если ключ подключён: «Распознать с ИИ» на подложке прочитает план вместе с подписями и сам поставит масштаб, а «✨» рядом с комнатой расставит мебель и объяснит каждое место в свойствах предмета.
...
7. **3D.** Кнопка «3D / AR»: объёмная сцена, «Стены: прозрачные» открывает вид сверху, клик по предмету выбирает его. Приблизьтесь колесом — у мебели видны спинки, ножки, ручки и конфорки.
...
| `underlay.ts` | Подложка-схема: загрузка картинки, калибровка масштаба и распознавание стен и проёмов на растре. |
