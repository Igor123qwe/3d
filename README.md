...
**Своя схема картинкой.** «Загрузить свой план» на стартовом экране (или просто перетащите картинку в окно, вставьте по Ctrl+V) кладёт скан БТИ или скрин объявления под чертёж. Масштаб задаётся кнопкой «Калибровать»: покажите на подложке отрезок с известным размером и введите его длину. Дальше стены можно обвести вручную с привязками или нажать «Распознать стены» — планировщик сам найдёт стены и дверные проёмы, а вы поправите результат.

**Каталог мебели.** 75 предметов с реальными размерами (поиск понимает «софа», «стиралка», «телек»): спальня, гостиная, кухня, санузел, прихожая, кабинет, детская, электрика. Объекты магнитятся спинкой к стене, выравниваются друг по другу, поворачиваются ручкой и масштабируются.
...
## Электрика: проект как у инженера

Режим «Электрика» (в шапке) — отдельный слой: мебель и стены видны, но мышь цепляет только точки электрики. В панели — «⚡ Спроектировать по нормам» и сам проект (`electrics.ts` — расстановка, `electricplan.ts` — инженерная часть). Это проект-эскиз для разговора с электриком и расчёта материалов, а не рабочая документация.

**Исходные данные**: выделенная мощность (от неё вводной автомат), плита — электрическая или газовая, высота потолка (по ней трасса).

**Точки — на стенах, по нормам.** Каждая розетка, выключатель, термостат садится на грань стены лицом в комнату; место в проёме, в зонах 0–2 ванны и душа (60 см, ГОСТ Р 50571.7.701), ближе 50 см к газовой плите или вплотную к мойке — точка сама едет вдоль стены, а если там нельзя — на соседнюю стену.
- Розеток — не меньше нормы (СП 31-110-2003, п. 14.29): в жилой комнате одна на каждые 4 м периметра, на кухне не меньше четырёх, в коридоре — одна на каждые 10 м². Сначала — у мебели (у изголовья по две с каждой стороны на 70 см, у дивана, над рабочим столом на 90, за телевизором на 120), потом добираются до нормы по свободным стенам — не за шкафами, как можно дальше друг от друга.
- Техника — на своих линиях и своей высоте: холодильник, посудомойка (в соседней тумбе), духовой шкаф, варочная панель (силовая розетка 32 А), вытяжка над плитой на 210, над столешницей — на 110, стиральная машина — рядом, не за ней, на 100, по желанию — кондиционеры у окон и водонагреватель. У газовой плиты — розетка поджига не ближе 50 см.
- Выключатели — у двери со стороны ручки на 90 см, у санузла — снаружи; в спальне — проходной у кровати.

**Щит и группы** — считаются из точек на каждую правку: сдвинули розетку — трасса, длина и нагрузка пересчитались.
- Свет — автомат B10, кабель ВВГнг(А)-LS 3×1,5; больше 10 точек — жилые комнаты отдельно от кухни, санузла и коридора.
- Розетки — дифавтомат C16 30 мА, 3×2,5, до 8 розеток и двух комнат на группу; кухня — рабочая зона отдельными группами по три розетки; санузел и стиральная машина — УЗО 10 мА.
- Техника на своих линиях: варочная панель — C32 и 3×6, остальное — C16 и 3×2,5.
- Нагрузка: номинал по назначению (чайник на столешнице — 1 кВт, варочная — 7 кВт…), для розеток общего назначения — коэффициент спроса, ток группы сверяется с автоматом (перегрузка — ошибка «разделите группу»). Ввод — по выделенной мощности (10 кВт → C50), реле напряжения, противопожарное УЗО; расчётная мощность квартиры сверяется с выделенной.
- Трассы — от щита по потолку (в 15 см от него) с изломами под прямым углом, к каждой точке — вниз по стене; длина с запасом 10 % и на разделку. На плане — пунктир цветом группы, номер группы в конце трассы, у точек — высота установки; щелчок по группе в щите — подсветка на плане. Щита нет на плане — место предложено в прихожей.
- Щит — число модулей на DIN-рейке и корпус с запасом 20 %.

**Материалы** — розетки (обычные, IP44 для санузла, силовая), выключатели, светильники и датчики, подрозетники, распаечные коробки, кабель по сечениям в метрах, автоматы и дифавтоматы по номиналам, щит. «Скачать щит и ведомость (CSV)» — таблица для Excel.

**Нормы** — замечания со ссылкой на точку: розеток меньше нормы, розетка в зоне ванны, у газовой плиты или мойки, выключатель ниже 80 или выше 170 см (ПУЭ, гл. 7.1), точка в проёме, за открытой дверью или не на стене, техника без своей линии, перегрузка группы, нагрузка больше выделенной. Замечания электрики видны и в общей «Проверке».

В свойствах точки — высота, назначение розетки (от него группа, автомат и кабель), мощность, группа щита и замечания к этой точке.
...
- **Несколько проектов в одном браузере**: «Проект» → «Мои проекты…» — список планов с датой и площадью, открыть, переименовать, скопировать, удалить. Старый одиночный план при первом запуске становится первым проектом. Проект, открытый во второй вкладке, предупредит об этом.
- **Несколько предметов разом**: Shift+клик добавляет предмет к выделению, Shift + рамка по пустому месту выделяет всё внутри, Ctrl+A — все предметы режима. Группу тянут за любой предмет, стрелки, Del, Ctrl+D и R действуют на всех; в панели — «↻ 90° все», «Дублировать все», «Удалить все».
- **Каталог не прыгает**: после постановки предмета вкладка «Каталог» остаётся — следующий предмет в один клик; карточку можно перетащить на план; Shift+клик и электрика оставляют предмет «в руке» до Esc; Enter ставит его в центр экрана без мыши.
- **Печать в масштабе** 1:50 и 1:100 («Проект» → «Печать в масштабе…»): лист A4 с масштабной линейкой, размеры на бумаге совпадают с линейкой. Экспорт PNG, SVG, JSON и CSV: щит с ведомостью, список покупок по товарам из магазинов.
- Все иконки векторные, в одном стиле; шрифт Inter с системным запасом.
- **Режимы «Стройка», «Мебель», «Электрика»** — переключатель в шапке, как «строительство» и «покупка» в The Sims. В каждом режиме мышь цепляет только свои объекты: в стройке — стены, двери, окна и размеры (мебель и электрика видны бледно), в мебели — предметы (стены не сдвинутся, пока двигаете диван), в электрике — розетки, выключатели, свет и щит. Слева — инструменты режима, справа — его панель. Инструмент стройки (W, D, N…) сам включает стройку; объект, выбранный в «Проверке» или в 3D, включает свой режим.
- **Стены правятся как в режиме строительства The Sims** (`walledit.ts`). Раньше стену тащили за кусок в любую сторону: кусок отрывался от соосных соседей (после распознавания одна прямая — несколько кусков), примыкающие стены перекашивались, стена в Т-стыке оставалась на месте — и комната размыкалась. Теперь:
  - **единица правки — прямая стена целиком**: наведение и выбор подсвечивают все соосные куски подряд;
  - **тянуть можно только поперёк** — со стрелками на стене, шагом сетки и привязкой «в линию» к соседней параллельной стене (Shift — без привязок); примыкающие стены, в том числе Т-стыки посреди прямой, тянутся за ней, углы остаются прямыми, комнаты — замкнутыми; во время перетаскивания видно «сдвиг N см»;
  - **Alt + тянуть — только участок** до ближайших стыков: по краям встают перемычки, так делается ниша или выступ; после этого выделение переходит на выдвинутый участок;
  - **стрелки — сдвиг на 1 см**, с Shift — на 10, всей прямой или выделенного участка; стрелка вдоль стены ничего не ломает, а подсказывает, какими стрелками её двигать;
  - **кружки на концах — длина**: поперечная стена за концом едет целиком (с Shift — свободно, как раньше); длина и толщина в панели свойств — тоже у всей прямой;
  - **Del и «Удалить стену» убирают прямую**, которая подсвечена. **Удалить часть стены**: Alt + щелчок выделяет участок до ближайших стыков (подпись — его длина), Del убирает только его — так делается проём в перегородке без двери; двери и окна на оставшейся части стоят где стояли;
  - **длина по оси, по внутренней или по наружной грани** — выбор в свойствах стены («Длину считать: по оси / внутри / снаружи») и в меню «Вид». Грань считается честно, от угла до угла: на внутреннем углу она короче оси на полтолщины поперечной стены, на наружном — длиннее (у стены 400 по оси с углами по 10 см: внутри 390, снаружи 410). Число, введённое по грани, пересчитывается в ось: разница задаётся стыками на концах и при растяжке не меняется. Если внутреннюю грань режут примыкающие перегородки, одного числа у неё нет — панель показывает отрезки («390 + 390»): это стороны разных комнат, их задают щелчком по размеру комнаты. У перегородки нет наружной грани — тогда длина по оси, и это сказано. На чертеже у выбранной стены подсвечена грань, по которой идёт подпись;
  - **размерные линии, привязанные к стенам** (`dims.ts`): инструмент «Размер» (M) ловит конец, ось или грань стены (кружок у курсора: красный — конец, зелёный — грань) и запоминает, к чему привязан конец. Между двумя параллельными гранями размер встаёт поперёк, даже если щёлкнули чуть вкось. После любой правки стен — сдвиг, растяжка, стрелки, склейка кусков — привязанный конец переезжает следом (история правок вызывает `followDims` на каждое изменение), так что размер «от стены до стены» всегда показывает текущее расстояние; Ctrl+Z возвращает и его. Удалили стену — размер остаётся на месте свободным. В свойствах комнаты — «Размеры комнаты на план»: ширина и глубина в чистоте, привязанные к граням. У выбранного размера зелёные точки — привязанные концы, кнопка «Отвязать от стен»;
  - **длина с клавиатуры, пока рисуете стену**: наберите цифры (в текущих единицах, запятая или точка) — у курсора видно «234 см · Enter — поставить», а резиновая линия уже этой длины; Enter ставит точку в сторону курсора (почти по оси — строго по оси), Backspace стирает цифру, Esc — набранное. Длина — по оси стены; точный размер в чистоте — щелчком по подписи комнаты;
  - **после каждой правки чертёж склеивается**: соосные куски встык одной толщины и без чужого стыка между ними — одна стена, стена нулевой длины уходит, наложенные стены сливаются, двери и окна переезжают на склеенную стену на прежнее место;
  - **если правка всё-таки разомкнула комнату**, это сказано сразу: «контур комнаты разомкнулся — Ctrl+Z вернёт».
  - **размер комнаты — числом**: щелчок по подписи стороны комнаты («230 см») открывает поле; введите 234 и выберите, какую стену двигать — левую или правую (верхнюю или нижнюю). Двигается только стена за этим углом, целиком и поперёк себя, а не вся длинная прямая, в которую входит сторона. По умолчанию — стена покороче и не зафиксированная: она задевает меньше чертежа;
  - **замок**: стену можно зафиксировать в её свойствах, а кнопкой «Замок» слева — все стены разом. Зафиксированную стену нельзя сдвинуть, растянуть, удалить или поменять ей толщину — ни мышью, ни числом, ни через соседнюю стену, что за неё держится; правка не применяется, и сказано почему. Двери, окна и мебель двигаются как обычно. На зафиксированных стенах — значок замка. Замок сохраняется вместе с планом и в файле проекта.
  - **разрывы между стенами**: стена, что не дошла до соседней на 30 см и меньше (или обе не дошли до угла), не даёт комнате замкнуться — комната пропадает с чертежа. «Проверка» показывает каждый такой разрыв, а кнопка «Замкнуть разрывы» дотягивает концы до соседних стен; зафиксированные стены не трогаются.
...
**Подпись на линии и ступенька под подписью.** На снимке пользователя (≈850 точек в ширину) после листа сошлись 4ж 401 × 426, кухня 330 × 426, коридор 277 / 137 и уступ 38, а ниша у входа осталась 108 × 38 со ступенькой. На похожем кадре «0,68» терялась при поиске: цифры лежат на низу ниши, рядом тонкая наружная стена в одну линию у края кадра и выносная черта — после утолщения всё слипалось в одно пятно не по размеру подписи. Теперь перед поиском из меток убираются прямые длиннее двух высот цифры (у знака штрих не длиннее строки), жирные цифры уменьшенной копии (залиты на 77 %) подписью считаются до 85 % заливки, а подпись из двух оставшихся знаков («70» от «2,70», чья «2» ушла со стенкой у двери) — тоже подпись; вырезка на листе вдоль строки шире, чтобы потерянный знак был виден. Если низ ниши на картинке распался ступенькой (38 + 25 со сдвигом 10 см), «0,68» не сходится ни с одной гранью. Такая ступенька под одной подписью — от цифр, а не от стены (`picture.ts: flattenLabelledSteps`): если сумма двух параллельных граней, соединённых короткой поперечной (до 20 см и четверти числа), сходится с числом, а по отдельности ни одна не сходится, грани встают на уровень длинной ещё до сборки стен. Настоящий уступ (0,64 × 0,13 на кухне) подписан по граням, каждая сходится со своим числом, и его это не трогает. В ступеньке участвуют и числа с листа, и подписи вдоль стен от модели: сторона и место вдоль неё дают точку на контуре (`pointOnOutline`). У пользователя лист «0,68» не прочитал, а модель по комнате прочитала и «1,29», и «0,68» — но они «не нашли своей стены»: низ ниши вышел 26 + 38 со сдвигом 13 см, правая стена ниши 108. После выпрямления низ ниши — одна грань 64, правая стена 122, и оба числа ложатся на свои грани.

**Место вдоль стены — по рамке комнаты, а не по вырезке** (`planai.ts: roomLabelInBox`). Модель видит комнату вырезкой с полями в 5 % кадра и долю вдоль стороны («at») называет по всей вырезке, а подписи и проёмы вставали по рамке комнаты. У ниши на правом краю прихожей ошибка выходила в полметра: «0,68» попадала мимо ниши, в коридор, а «1,29» не получала послабления «число стоит точно у своей грани» и «не находила своей стены». Теперь доля пересчитывается из вырезки (обрезанной краем картинки, как в `cropForVision`) в рамку комнаты — для подписей вдоль стен и для проёмов.

**Ниша, суженная цифрами у стены.** Кадр пользователя удалось собрать в его разрешении (851 точка, 1,42 см в точке — план с белыми полями со скриншота, bti-dvushka-user-851). На нём цифры «1,29», стоящие боком у правой стены ниши, слипаются со стеной и выносной чертой в столбец в 25 точек: правая стена ниши сдвигается внутрь, ниша выходит 47 см вместо 0,68, и ни одна грань не сходится с числом даже с запасом 20 %. Теперь короткое число (до 1,5 м) может встать на грань, расходящуюся с ним до 45 %, — если такая грань у этой стороны комнаты одна (у числа с листа — одна рядом с ним); подгонка растягивает нишу, а лишнюю толщину снимает со «стены» из цифр. Ступенька по подписи модели, чьё место вдоль стены ненадёжно, выпрямляется без места — только если такая ступенька в контуре одна. Без этого на кадре пользователя низ ниши выходил 61, с этим — 129 × 68, и все грани прихожей сходятся с планом.
...
**Расстановка мебели по пожеланиям — вся квартира или комната** (`furnish.ts`, `FurnishDialog.tsx`). В режиме «Мебель» — «✨ ИИ» слева (и кнопка в каталоге, и «Обставить с ИИ…» в свойствах комнаты). В диалоге — пожелания своими словами («двое взрослых и ребёнок, нужен кабинет, много хранения»; готовые фразы дописываются щелчком), где расставлять и что делать с мебелью, что уже стоит: дополнить (модель видит её и не ставит поверх) или убрать (электрика остаётся).
- **Вся квартира** — два шага. Сначала короткий вопрос о зонировании: какой комнате какое назначение по вашим пожеланиям (спальни и детская — в тихих непроходных, детская — с окном и от 8 м², не хватает комнат — совмещать и сказать об этом). Кухню, санузел и прихожую модель не переназначает — там разводка. Потом по вопросу на комнату (по три разом): назначение, пожелания, остальные комнаты квартиры (обеденный стол — один на квартиру) и то, что уже стоит. Комнаты можно переименовать по назначению.
- **Комната** — назначение задаётся руками («Кабинет», «Гостевая»…), зонирования нет.
- **Как расставлять** — выбор в диалоге. «Тщательно» (по умолчанию): сильная модель сначала пишет замысел комнаты — где спать, хранить, работать, где проход от двери, откуда свет — и расставляет по нему; замысел виден в отчёте. Около 3–4 ₽ за комнату. «Быстро»: дешёвая модель, копейки, но думает меньше. Выбор запоминается.
- Модели уходит чистовой контур комнаты от её угла, x и y — центр предмета, rot — куда смотрит спинка; если модель всё же ответит углами, программа это распознаёт.
- Координаты модели приблизительные, поэтому ни один предмет не попадает в план без проверки геометрией, и не выбрасывается зря: не подошло место — ищется ближайшее честное, спинкой к стене, мимо двери, окна и соседей; нигде не встаёт — тот же предмет поменьше; отказ — только если места нет вовсе.
- Затем второй проход, как у дизайнера: каждый предмет пересматривается с учётом соседей. Двуспальная кровать — по центру и не меньше 60 см с каждой стороны, тумбы едут вместе с ней; распашной шкаф, которому не хватает места на дверцы, становится купе; шкаф растягивается на всю нишу или стену до 3 м; столик и стол — у окна боком к свету; высокое не закрывает окно. Кровати без тумб они добавляются у изголовья, если есть место. В спальне есть стол со стулом.
...
| **Расстановка мебели — «Тщательно»** (по умолчанию) | `anthropic/claude-sonnet-5` — продумывает зоны, проходы и свет | 218,9 / 1094,6 | 6000 |
| | `openai/gpt-5-mini` | 27,4 / 218,9 | |
| | `deepseek/deepseek-v4-flash` | 4,5 / 9,0 | |
| **Расстановка мебели — «Быстро»** | `deepseek/deepseek-v4-flash` | 4,5 / 9,0 | 4000 |
...
Запускалка проверяет не только папку `node_modules`, но и сам Vite внутри: если его нет (установка прервалась, в системе стоит `NODE_ENV=production`, на месте папки лежит файл), она доставит зависимости сама. Vite запускается через Node напрямую, поэтому ошибки «'vite' is not recognized» в Windows больше не бывает. Если что-то всё же пошло не так — удалите папку `node_modules` и запустите `start.cmd` ещё раз.
...
**Автоматические тесты.** `npm test` — около 600 проверок геометрии, поиска комнат, эргономики, операций над планом, истории правок, электрики, проектов и разбора плана из ссылки. `npm run typecheck` проверяет типы. `npm run build && npm run smoke` открывает собранное приложение в настоящем браузере (playwright-core, `npx playwright-core install chromium`): стартовый экран, шаблон, каталог, электрика — без ошибок страницы. Всё это выполняется в CI на каждый push; демо на GitHub Pages выкатывается только после зелёного CI.
...
| `autolayout.ts` | Проверка и починка расстановки: честное место спинкой к стене мимо двери и соседей, правила дизайнера, второй проход, шкаф во всю нишу. |
...
| `tools/start.mjs` | Обновить из git, доставить зависимости и запустить. Если порт занят прежней копией — предупредить, взять свободный и открыть браузер именно на нём (раньше сервер молча уходил на 5174, а браузер открывался на старой копии с 5173). |
...
| `Del` | Удалить |
| `Ctrl+Z`, `Ctrl+Y` | Отменить, вернуть (и в 3D) |
| `Esc`, `Enter` | Завершить стену, отменить инструмент; `Enter` при постановке — предмет в центр экрана |
| `Backspace` | Снять последнюю точку черновика стены |
| `Shift+клик`, `Shift`+рамка, `Ctrl+A` | Несколько предметов: добавить, выделить рамкой, все в режиме |
| `Alt+клик` | Участок стены до стыков (Del и «Удалить участок» убирают только его) |
| `+` `−` `0` | Приблизить, отдалить, показать весь план |
| `Ctrl+S`, `Ctrl+O`, `Ctrl+V` | Сохранить в файл, открыть, вставить картинку или план |

## Статус

Работает и покрыто тестами: геометрия, поиск комнат, привязки, проверки эргономики и норм СП 54.13330, операции над планом, история, проекты, электрика, передача плана по ссылке; дымовая проверка в браузере — в CI.

Что сделано по аудиту против лидеров (Planner 5D, Floorplanner, HomeByMe, RoomSketcher, Sweet Home 3D, Remplanner) и что осталось — в [docs/audit.md](docs/audit.md).
