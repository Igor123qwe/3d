...
- Координаты модели приблизительные, поэтому каждый предмет проверяется геометрией: вылез за стену, перекрыл дверь, наехал на соседа — отброшен с причиной. В план попадает только проверенное, **одной правкой**: Ctrl+Z убирает расстановку всей квартиры разом. После прогона — отчёт по комнатам: назначение и почему, сколько поставлено, что отброшено и почему.
