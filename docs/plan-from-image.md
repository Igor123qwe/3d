...
| Автоматика | Нейросеть сегментирует комнаты по пикселям (CubiCasa5k, DeepFloorplan, VectorFloorSeg), модель со зрением читает подписи | То же разделение труда, но без обучения и весов: **комнаты находятся классической сегментацией** очищенного растра (заливка с закрытием проёмов, радиус — голосованием по масштабам, поле листа срезается; каждая комната обводится по пикселям многоугольником с прямыми углами, стены собираются из граней соседей), **модель к ним только подписывает** имя и площадь — по точке или по совпадению площадей; чертёж строится заново по размерам с проверкой площадей; выдуманные комнаты отпадают сами, слабый ответ перепроверяется моделью посильнее |
...
| Источник геометрии | Нейросеть по пикселям или модель «по координатам» | Если комнаты нашлись на картинке — **только картинка**: стены по граням областей, толщина по щели, цифры стен не двигают. Цифры задают масштаб по согласию и проверяются, несошедшиеся названы спорными и перечитаны |
| Чтение подписей | Один проход по всему снимку | **Комната за комнатой**: геометрия уже есть с картинки, поэтому модель смотрит на каждую комнату отдельно, увеличенной. Мелкие цифры на общем снимке путаются и уводят масштаб |
...
Честно о пределах: комнаты с картинки — многоугольники с прямыми углами, а по
клику и по числам — прямоугольники; скошенные стены и эркеры под углом выходят
ступеньками и правятся руками; выступ наружу мельче радиуса закрытия проёмов
(≈ 45 см) срезается как язычок в проёме; сегментация требует замкнутых стен на картинке — где линия прервана,
...
- Классика «заливка → контур → многоугольник» на форумах и в открытых проектах: [OpenCV Forum: Find contours for walls in floor plan images](https://forum.opencv.org/t/find-contours-for-walls-in-floor-plan-images/11640), [Autonomous Room Detection in Floor Plans With OpenCV](https://ivanovi.ch/blog/2019/12/how-heatmap-works-3/) (заливка комнаты от подписи), [FloorplanToBlender3d](https://github.com/grebtsew/FloorplanToBlender3d/blob/master/FloorplanToBlenderLib/detect.py) (контуры и approxPolyDP), [py_floor_plan_segmenter](https://github.com/sharif1093/py_floor_plan_segmenter) (сравнение морфологии, преобразования расстояния, Вороного и понижения разрешения), [Floor-SP](https://github.com/woodfrog/floor-sp) (план как набор многоугольников комнат)
