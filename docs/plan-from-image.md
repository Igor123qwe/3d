...
| Автоматика | Стены с картинки, «все линии — стены» | **ИИ читает числа и соседство, геометрия берётся с картинки**: рамки комнат привязываются к стенам заливкой, чертёж строится заново по размерам с проверкой площадей; выдуманные комнаты выбрасываются, слабый ответ перепроверяется моделью посильнее |
