...
describe('прямые углы без мелочи', () => {
  ...
  it('ступенька в углу от цифр на стене («0,68» в нише) выравнивается, уступ со стеной остаётся', () => {
    ...
    // цифры «0,68» слиплись со стеной и попали в стеновые чернила — пятно, а не линия: угол заполняется
    expect(fillDents(step, 20, (x, y) => y >= 80 || x >= 100 || (x >= 78 && x <= 92 && y >= 74 && y <= 77))).toEqual(rect(0, 0, 100, 80))
  })
})
...
describe('ступенька под одной подписью', () => {
  ...
  it('подпись от модели — сторона и место — встаёт на контур и выпрямляет так же (у пользователя: 26 + 38, сдвиг 13)', () => {
    const user: Pt[] = [
      { x: 0, y: 0 },
      { x: 360, y: 0 },
      { x: 360, y: 109 },
    ...
    // «0,68» у низа выреза, почти у правого края комнаты
    const at = pointOnOutline(user, 'bottom', 0.93)
    const got = flattenLabelledSteps(user, [{ at, alongX: true, cm: 68 }], 1)
    expect(got).toEqual([
      { x: 0, y: 0 },
      { x: 360, y: 0 },
      { x: 360, y: 122 },
  ...
  it('подпись модели без точного места — только если ступенька в контуре одна', () => {
    const far = { at: { x: 100, y: 300 }, alongX: true, cm: 68, loose: true }
    // место неверно (посреди комнаты), но ступенька под числом одна — выпрямляется
    expect(flattenLabelledSteps(niche, [far], 1)).toHaveLength(6)
    // две одинаковые ступеньки — не угадать, какую
    const twice: Pt[] = [
      { x: 0, y: 0 },
      { x: 360, y: 0 },
      { x: 360, y: 100 },
    ...
    expect(flattenLabelledSteps(twice, [{ ...far, cm: 63 }], 1)).toBe(twice)
  })
