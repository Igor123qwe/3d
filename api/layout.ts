...
Сначала продумай комнату целиком, потом ставь: где спать, где хранить, где работать или отдыхать, где проход от двери к кровати, окну и шкафу, откуда свет. Запиши этот замысел коротко в plan — 2–4 предложения, потом расставь по нему.

Верни ТОЛЬКО JSON: {"plan":"замысел комнаты","items":[{"type":"ключ из каталога","x":320,"y":180,"rot":90,"why":"зачем именно тут"}]}
...
- В спальне от 12 м² с окном — рабочее место у окна (стол и стул), если пожелания не против и оно не отнимает проходов.
- Шкаф займёт всю нишу или стену, где встал, — программа подгонит ширину сама; ставь его туда, где перед ним останется место открыть дверцы (распашному 90 см, купе 60 см).
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    const answer = await askJson(cfg, {
      // тщательно — сильная модель; быстро — прежняя дешёвая цепочка
      task: body.quality === 'fast' ? 'layout' : 'layout_pro',
      // замысел модели — пользователю в отчёт: видно, почему так
      check: (d: unknown) => {
        const plan = (d as { plan?: unknown } | null)?.plan
        return { items: checkAiLayout(d), plan: typeof plan === 'string' ? plan.trim().slice(0, 600) : undefined }
      },
      messages: [
    ...
    return json({ items: answer.value.items, plan: answer.value.plan, ai: { model: answer.model, costRub: answer.costRub, tried: answer.tried } })
  } catch (e) {
    return fail((e as Error).message, 502)
  }
}
