...
export interface AiRoom {
  ...
  exact?: boolean
  /** толщина стены за каждой стороной, измеренная по картинке, в пикселях */
  wallPx?: Partial<Record<AiSide, number>>
  /** стороны, упёршиеся в край кадра: там фото обрезано, и размер можно взять только с подписи */
  cut?: AiSide[]
}
